...
from chess.cad_model import CadDiagnostic, CadMetadataError
from chess.cad_system import (
    CadSystem,
    SystemCandidate,
    discover_systems,
...
from chess.cad_planning import CadSelection, build_render_plan, plan_as_dict
from chess.cad_profiles import (
...
from chess.context import ProjectContext
    ...
    if selector is not None:
        ...
        try:
            ...
            raise ValueError(
                ...
                f"is not available; run 'chess cad systems' and choose one of: "
                f"{choices or '(none)'}"
            ) from None
...
def _selected_system(
    ...
    if candidate.status != "valid":
        if configured is not None:
            ...
            raise ValueError(
                f"configured default_system {configured!r} in {preferences_path} "
                f"is invalid; run 'chess cad systems' and choose one of: "
                f"{choices or '(none)'}"
            )
...
def _system_rows(context: ProjectContext, deps: Mapping[str, CadFunction]) -> list[dict[str, object]]:
    rows = []
...
def _catalog_rows(action: str, system: CadSystem, context: ProjectContext,
...
def _prepare_system_plan(
    args: argparse.Namespace, context: ProjectContext, deps: Mapping[str, CadFunction],
    ...
    try:
        ...
        if args.action == "new":
            ...
            if args.json:
                ...
                stdout.write(
                    f"chess cad sets {created.model_id} --system {system.name}\n"
                )
            return 0
