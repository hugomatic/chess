...
from chess.cad_model import CadMetadataError as CadModelMetadataError, load_model
from chess.cad_system import CadSystem, load_system


_SAFE_NAME = re.compile(r"^[A-Za-z0-9_-]+$")
_SCAD_IDENTIFIER = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")
_TOKEN = "__CLAW_MODEL__"
...
def _exchange_paths(first: Path, second: Path) -> None:
    ...
    from chess.cad_fs import AtomicExchangeUnsupported, exchange_paths
    try:
...
def _remove_owned_directory(path: Path, identity: tuple[int, int]) -> bool:
    ...
    from chess.cad_fs import AtomicExchangeUnsupported, remove_owned_path
    try:
