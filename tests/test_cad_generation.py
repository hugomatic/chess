...
from chess.cad_generation import (
    CadRunExistsError,
    generate_plan,
    list_runs,
    load_job_log,
...
from chess.cad_model import CadModel, CadSet
from chess.cad_planning import CadSelection, RenderJob, RenderPlan, ResolvedProfile
from chess.cad_manufacturing import DirectiveSource, merge_manufacturing
from chess.cad_values import ResolvedVariable, VariableLayer
...
class CadGenerationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.fake.write_text(textwrap.dedent("""\
            ...
            if state_log and os.environ.get("CHESS_CAD_MANIFEST"):
                manifest = json.loads(pathlib.Path(os.environ["CHESS_CAD_MANIFEST"]).read_text())
    ...
    def test_resolve_openscad_uses_platform_fallbacks_in_order(self):
        ...
        with mock.patch("chess.cad_generation._is_executable",
        ...
        for expected in linux_paths:
            with self.subTest(expected=expected), mock.patch(
                "chess.cad_generation._is_executable",
    ...
    def test_resolve_openscad_missing_or_unsupported_is_humane(self):
        for system in ("Linux", "Plan9"):
            with self.subTest(system=system), mock.patch(
                "chess.cad_generation._is_executable", return_value=False
    ...
    def test_run_ids_are_human_readable(self):
        ...
        with mock.patch("chess.cad_generation._local_now", return_value=instant):
    ...
    def test_same_day_duplicate_is_rejected_before_openscad(self):
        ...
        with mock.patch(
            "chess.cad_generation._local_now",
    ...
    def test_duplicate_identity_allows_different_jobs_or_local_day(self):
        ...
        with mock.patch("chess.cad_generation._local_now", side_effect=times):
            first = self.generate(plan("first"))
    ...
    def test_regeneration_publication_failure_rolls_back_old_run(self):
        ...
        with mock.patch("chess.cad_generation._local_now", side_effect=times):
            ...
            with mock.patch(
                "chess.cad_generation.os.replace",
    ...
    def test_manufacturing_change_reuses_geometry_with_fresh_archive_metadata(self):
        ...
        self.argv_file.unlink()
        zone = timezone(timedelta(hours=-10))
        with mock.patch("chess.cad_generation._local_now", return_value=datetime(
    ...
    def test_reuse_copies_verified_descriptor_not_swapped_path(self):
        import chess.cad_generation as generation
        first = self.generate(plan("first"))
        manifest = load_run(first.run_dir)
        artifact = first.run_dir / manifest["jobs"][0]["artifact"]
        ...
        with mock.patch("chess.cad_generation._copy_verified_artifact",
    ...
    def test_reuse_publication_never_overwrites_concurrent_target(self):
        ...
        with mock.patch("chess.cad_generation.os.link", side_effect=inject_target):
    ...
    def test_post_claim_run_swap_before_source_never_writes_replacement(self):
        ...
        with mock.patch("chess.cad_generation._copy_snapshot",
    ...
    def test_post_claim_run_swap_before_manifest_never_writes_replacement(self):
        ...
        with mock.patch("chess.cad_generation._write_manifest",
    ...
    def test_post_claim_run_swap_before_render_never_starts_openscad(self):
        ...
        self.argv_file.unlink(missing_ok=True)
        with mock.patch("chess.cad_generation._write_readme",
    ...
    def test_popen_boundary_uses_anchored_source_output_and_swap_cleanup(self):
        ...
        with mock.patch("chess.cad_generation.subprocess.Popen",
    ...
    def test_descriptor_anchoring_fails_safely_without_proc(self):
        ...
        try:
            with mock.patch("chess.cad_generation.Path.is_dir", return_value=False), \
                    self.assertRaisesRegex(RuntimeError, "requires Linux /proc"):
                __import__("chess.cad_generation", fromlist=["_proc_fd_path"])._proc_fd_path(
    ...
    def test_interrupted_job_records_elapsed_time_before_reraising(self):
        ...
        with mock.patch(
            "chess.cad_generation._capture_line", side_effect=KeyboardInterrupt
        ):
    ...
    def test_catalog_is_newest_first_and_log_uses_exact_artifact_id(self):
        ...
        with mock.patch("chess.cad_generation._utc_now", return_value=old_time):
            older = self.generate(
                output=self.data / "cad" / "prints" / "fixture-system" / "older"
            )
        with mock.patch("chess.cad_generation._utc_now", return_value=new_time):
            newer = self.generate(
