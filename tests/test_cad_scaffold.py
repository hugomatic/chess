...
from chess.cad_model import CadDiagnostic, CadMetadataError, load_model
from chess.cad_scaffold import (
...
SCAD = '''set = "__CLAW_MODEL__"; // [__CLAW_MODEL__, assembly]
module __CLAW_MODEL___positive() { cube(1); }
module __CLAW_MODEL___negative() {}
module __CLAW_MODEL__() { difference() { __CLAW_MODEL___positive(); __CLAW_MODEL___negative(); } }
if (set == "__CLAW_MODEL__") { __CLAW_MODEL__(); }
else if (set == "assembly") { __CLAW_MODEL__(); }
'''


def sidecar(description="General model"):
    return {
        "schema": "plamp-cad-model/1", "name": "__CLAW_MODEL__",
        "source": "__CLAW_MODEL__.scad", "description": description,
        "sets": {
            "__CLAW_MODEL__": {"description": "Printable model"},
...
class CadScaffoldTests(unittest.TestCase):
    ...
    def test_staging_failures_leave_no_model_or_stage(self):
        for target, failure in (
            ...
            with self.subTest(target=target), mock.patch(
                f"chess.cad_scaffold.{target}", side_effect=failure
            ):
    ...
    def test_template_replacement_symlink_race_is_rejected(self):
        ...
        def raced(_root):
            ...
            return discovered
        with mock.patch("chess.cad_scaffold.discover_templates", side_effect=raced):
            with self.assertRaises(OSError):
                create_model(self.root, self.system, "pump", "cad")
        self.assertFalse((self.root / "things" / "pump").exists())
    ...
    def test_discovery_rejects_sidecar_inode_replacement_before_description_read(self):
        ...
        from chess import cad_scaffold
        ...
        def race(path):
            ...
            return identity
        with mock.patch("chess.cad_scaffold._regular_identity", side_effect=race):
            with self.assertRaises(OSError):
                discover_templates(self.root)

    def test_manifest_replace_failure_rolls_back_published_model(self):
        original = self.system_path.read_bytes()
        with mock.patch("chess.cad_scaffold._replace_system_manifest", side_effect=OSError("disk full")):
            with self.assertRaises(OSError):
                create_model(self.root, self.system, "pump", "cad")
        self.assertFalse((self.root / "things" / "pump").exists())
        self.assertEqual(self.system_path.read_bytes(), original)
    ...
    def test_rollback_does_not_delete_concurrent_destination_replacement(self):
        ...
        with mock.patch("chess.cad_scaffold._replace_system_manifest", side_effect=replace_then_fail):
            with self.assertRaises(OSError):
                create_model(self.root, self.system, "pump", "cad")
    ...
    def test_atomic_rollback_claim_restores_swap_at_claim_boundary(self):
        ...
        with mock.patch("chess.cad_scaffold._replace_system_manifest", side_effect=OSError("fail")), \
             mock.patch("chess.cad_fs.exchange_at", side_effect=race):
            with self.assertRaises(OSError):
                create_model(self.root, self.system, "pump", "cad")
        self.assertEqual((destination / "sentinel").read_text(), "unrelated")
        self.assertFalse(moved.exists())
    ...
    def test_claimed_cleanup_verifies_open_descriptor_before_unlink(self):
        ...
        with mock.patch("chess.cad_scaffold.os.open", side_effect=race):
            with self.assertRaises(OSError):
    ...
    def test_final_removal_exchange_survives_post_cleanup_swap(self):
        ...
        with mock.patch("chess.cad_scaffold._replace_system_manifest", side_effect=OSError("fail")), \
             mock.patch("chess.cad_fs.remove_owned_entry_at", side_effect=swap_then_remove):
            with self.assertRaises(OSError):
                create_model(self.root, self.system, "pump", "cad")
    ...
    def test_unsupported_atomic_exchange_does_not_mask_manifest_failure(self):
        ...
        original = self.system_path.read_bytes()
        with mock.patch(
            "chess.cad_scaffold._replace_system_manifest",
            side_effect=OSError(errno.EIO, "manifest failure"),
        ), mock.patch(
            "chess.cad_scaffold._exchange_paths",
    ...
    def test_prospective_manifest_failure_prevents_publication(self):
        ...
        published = []
        with mock.patch("chess.cad_scaffold.load_system", side_effect=reject_second), \
             mock.patch("chess.cad_scaffold._publish_noreplace", side_effect=lambda *args: published.append(args)):
    ...
    def test_publish_race_preserves_competing_directory_and_manifest(self):
        ...
        with mock.patch("chess.cad_scaffold._publish_noreplace", side_effect=race):
