...
from chess.cad_dependencies import (
    CadLibrary,
    CadDependencyError,
    DependencyRecord,
    DependencyClosure,
...
from chess.cad_generation import _command
...
"""
    ...
    @patch("chess.cad_dependencies.subprocess.run")
    ...
    def test_dirty_discovery_detects_source_replacement_during_validation(self):
        ...
        with patch("chess.cad_dependencies.os.open", side_effect=replace_before_open):
    ...
    def test_dirty_discovery_reads_anchored_source_after_path_replacement(self):
        ...
        with patch("chess.cad_dependencies.subprocess.run", side_effect=swap_at_launch):
            run_dependency_discovery(
    ...
    def test_archive_rejects_traversal_and_links_and_cleans_temporary_root(self):
        ...
        for kind in ("traversal", "symlink"):
            ...
            with self.subTest(kind=kind), patch(
                "chess.cad_dependencies.subprocess.run", return_value=completed
            ), patch(
                "chess.cad_dependencies.shutil.rmtree", wraps=shutil.rmtree
            ) as remove:
    ...
    def test_cleanup_tracks_owned_inode_across_validation_deletion_swap(self):
        ...
        from chess import cad_dependencies
        ...
        with patch(
            "chess.cad_dependencies._clear_cleanup_descriptor",
    ...
    def test_atomic_child_cleanup_tracks_moved_directory_and_file(self):
        ...
        for kind in ("directory", "file", "symlink"):
            ...
            try:
                with self.subTest(kind=kind), patch(
                    "chess.cad_fs.exchange_at", side_effect=swap_at_claim
                ):
    ...
    def test_atomic_child_cleanup_unlinks_symlink_without_following_it(self):
        from chess.cad_fs import remove_owned_entry_at
