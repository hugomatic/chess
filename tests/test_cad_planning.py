...
from chess.cad_profiles import CadProfile, CadProfileError
from chess.cad_planning import (
    CadSelection,
    build_render_plan,
    plan_as_dict,
)
from chess.cad_values import resolve_variables
from chess.cad_system import CadProduct, CadProductItem, CadSystem, load_system
...
class CadPlanningTests(unittest.TestCase):
    ...
    def test_repository_board_expands_exact_model_and_set(self):
        repo_root = Path(__file__).resolve().parents[1]
        system = load_system(repo_root / "cad" / "chess.system.cad.json", repo_root)
        plan = build_render_plan(
            system,
            CadSelection(product="board"),
