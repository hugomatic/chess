...
from chess.cad_model import CadMetadataError
from chess.cad_dependencies import CadLibrary
from chess.cad_system import discover_systems, load_system, select_system


class CadSystemTests(unittest.TestCase):
    def test_repository_chess_system_has_exact_models_and_products(self):
        repo_root = Path(__file__).resolve().parents[1]
        system = load_system(repo_root / "cad" / "chess.system.cad.json", repo_root)
        self.assertEqual(tuple(system.models), ("chess_board", "infaltable_boat"))
        self.assertEqual(
            tuple(system.products),
            ("board", "drawer", "assembly", "infaltable_boat"),
        )
        ...
        expected_items = {
            ...
            "infaltable_boat": (
                ("infaltable_boat", "scale_hull", None),
                ("infaltable_boat", "scale_floor", None),
                ("infaltable_boat", "scale_transom", None),
            ),
        }
